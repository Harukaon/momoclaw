const en: Record<string, string> = {
  "chat.placeholder": "Type a message...",
  "chat.send": "Send",
  "chat.stop": "Stop",
  "chat.new": "New Chat",
  "chat.you": "You",
  "chat.assistant": "Assistant",
  "chat.system": "System",
  "chat.empty": "Start a conversation...",
  "history.title": "History",
  "history.empty": "No conversations yet",
  "history.delete_confirm": "Delete this conversation?",
  "history.rename": "Rename",
  "history.delete": "Delete",
  "config.title": "Settings",
  "config.saved": "Settings saved",
  "config.provider": "Provider",
  "config.api_key": "API Key",
  "config.api_key_or_env": "API key or leave empty to use env var",
  "config.base_url": "Base URL",
  "config.base_url_hint": "Leave empty to use official endpoint",
  "config.api_type": "API Type",
  "config.api_type_auto": "Auto-set for built-in provider",
  "config.auth_env_vars": "Set environment variables",
  "config.or": "or",
  "config.aws_region": "AWS Region",
  "config.azure_resource": "Azure Resource Name",
  "config.azure_deployment": "Azure Deployment Name",
  "config.gcp_project": "GCP Project",
  "config.gcp_location": "GCP Location",
  "config.oauth_hint": "This provider uses OAuth device flow. Authenticate via CLI or set env var.",
  "config.oauth_env": "Environment variable",
  "config.oauth_authenticated": "Authenticated",
  "config.oauth_not_authenticated": "Not authenticated",
  "config.oauth_cli_hint": "Login via CLI /login command",
  "config.oauth_expires": "Expires",
  "config.optional": "Optional",
  "config.active_provider": "Active Provider",
  "config.default_model": "Default Model",
  "config.select_model": "Select a model",
  "config.model_placeholder": "Model ID (e.g. gpt-4o)",
  "config.fetch_models": "Fetch",
  "config.system_prompt": "System Prompt",
  "config.locale": "Language",
  "config.add_provider": "Add Provider",
  "config.remove_provider": "Remove Provider",
  "config.provider_name": "Provider Name",
  "config.form_mode": "Form",
  "config.json_mode": "JSON",
  "config.save": "Save",
  "config.cancel": "Cancel",
  "config.validation_error": "Validation error",
  "config.invalid_json": "Invalid JSON",
  "tool.running": "Running",
  "tool.done": "done",
  "tool.error": "error",
  "tool.awaiting_approval": "Awaiting approval",

  // Approval
  "approval.prompt": "Command requires approval:",
  "approval.allow": "Allow",
  "approval.allow_once": "Allow this command once",
  "approval.always": "Always Allow",
  "approval.always_desc": "Add to session whitelist",
  "approval.deny": "Deny",
  "approval.denied": "Command denied by user",
  "approval.timeout": "Approval timed out",
  "sidebar.main_agent": "Main Agent",
  "sidebar.sub_agents": "Sub-Agents",
  "sidebar.new_sub_agent": "New Sub-Agent",
  "sidebar.cannot_delete_main": "Cannot delete main agent",
  "error.prefix": "Error",
};

export default en;
